	...
	case TYPEC_ACCESSORY_AUDIO:
		...
		if (fsa_priv->vendor != DIO4480) {
			...
			if (fsa_priv->vendor == DIO4483) {
				...
				regmap_write(fsa_priv->regmap, DIO4483_DELAY_L_R, 0x0f);
...
static int typec_switch_to_fast_charger(struct fsa4480_priv *fsa_priv, unsigned long to_fast_charger)
{
	...
	if ((fsa_priv->vendor != WAS4783) && (fsa_priv->vendor != DIO4483) && (fsa_priv->vendor != HL5281)) {
		...
		do {
			...
			if (rc) {
				...
				mm_fb_audio_fatal_delay(OPLUS_AUDIO_EVENTID_HEADSET_DET, MM_FB_KEY_RATELIMIT_30MIN, \
					FEEDBACK_DELAY_60S, "charge glink notifier registration failed, chg_retries=%d", chg_retries);
...
/*add for 3rd protocal stack notifier*/
	...
	if (fsa_priv->vendor == DIO4483 && fsa_priv->chg_registration) {
		unregister_chg_glink_notifier(&fsa_priv->chg_nb);
	}
	/*add HL5281 support*/
	if (fsa_priv->vendor == HL5281 && fsa_priv->chg_registration) {
		unregister_chg_glink_notifier(&fsa_priv->chg_nb);
	}
